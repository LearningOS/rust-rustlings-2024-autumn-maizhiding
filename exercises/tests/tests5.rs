...
unsafe fn modify_by_address(address: usize) {
    // SAFETY: The caller must ensure that `address` is a valid and mutable reference
    // to a `u32` value. Here we are converting the address to a mutable pointer,
    // dereferencing it, and then modifying its value. If the `address` is invalid
    // or does not point to a valid `u32`, this could lead to undefined behavior.
    unsafe {
